...
const normalizeUploadRow = (row) => {
  ...
  return {
    user_id: anonymizeId(row.user_id, 'user'),
    os_type: row.os_type || row.platform || 'Unknown',
    card_id: anonymizeId(row.card_id, 'card'),
    plan_id: anonymizeId(row.plan_id, 'plan'),
    ...
    date: parseDate(row.timestamp || row.created_at || row.date)
  };
};
...
export const sanitizeCsvForStorage = (csvString) => {
  ...
  const rows = parsed.data.map(normalizeUploadRow).map(row => ({
    ...
    conversion_status: row.conversion_status,
    timestamp: row.date && !isNaN(row.date.getTime()) ? row.date.toISOString() : ''
  }));
...
export const calculateMetrics = (data) => {
  ...
  Object.values(userMap).forEach(userBookings => {
    ...
    const planIds = new Set(sorted.map(b => b.plan_id).filter(Boolean));
    const cardIds = new Set(sorted.map(b => b.card_id).filter(Boolean));
    const planCount = planIds.size || sorted.length;
    const cardCount = cardIds.size || (sorted.some(b => b.card_id) ? 1 : 0);
    const isRepeat = planCount > 1;
    ...
    if (isRepeat) {
      ...
      const cardSequence = sorted.map(b => getCardLabel(b.card_id)).join(' → ');
      const planSequence = sorted.map((_, idx) => `Plan ${idx + 1}`).join(' → ');
      ...
      if (diffDays === 0) {
        if (first.plan_id && second.plan_id && first.plan_id === second.plan_id) {
           pattern = `Warning: same plan ID repeated on the same day. Likely duplicate or retry, not a new plan.`;
        } else if (amt1 === amt2 && first.tenure === second.tenure && first.card_id === second.card_id) {
           pattern = `⚠️ Warning: Exact duplicate plan (Rs. ${formatShort(amt1)}, ${first.tenure}M) on the same day. Possible system retry bug.`;
        } else {
           const cardText = first.card_id !== second.card_id ? ' on another card' : '';
           pattern = `Same-day top-up${cardText}: Took a 2nd EMI plan of Rs. ${formatShort(amt2)} (${second.tenure}M) alongside the first Rs. ${formatShort(amt1)}.`;
        }
      } else if (diffDays !== null) {
        if (first.plan_id && second.plan_id && first.plan_id === second.plan_id) {
           pattern = `Same plan ID appears again after ${diffDays} days; treat as possible duplicate before calling it repeat behavior.`;
        } else if (amt1 === amt2 && first.tenure === second.tenure) {
           pattern = `Returned after ${diffDays} days for the exact same plan (Rs. ${formatShort(amt1)}, ${first.tenure}M).`;
        } else {
           const pctChange = amt1 > 0 ? Math.round(((amt2 - amt1) / amt1) * 100) : 0;
           const direction = pctChange > 0 ? "larger" : pctChange < 0 ? "smaller" : "different";
           const cardText = first.card_id !== second.card_id ? ' using another card' : '';
  ...
  if (repeatAnalytics.totalRepeatUsers > 0) {
    repeatAnalytics.avgDaysBetweenPlans = Math.round(totalDaysBetween / repeatAnalytics.totalRepeatUsers);
  ...
  data.forEach(row => {
    ...
    if (row.user_id) osBuckets[os].users.add(row.user_id);
  ...
  const ticketInsight = `Pattern: 50% of users use EMI plans below ${formatCurrency(bottom50MaxTicket)}, yet the top 50% of the total portfolio value is driven exclusively by ticket sizes above ${formatCurrency(topValueThreshold)}.`;
  ...
  Object.values(userMap).forEach(userBookings => {
    const distinctPlans = new Set(userBookings.map(b => b.plan_id).filter(Boolean));
    const isRepeatUser = (distinctPlans.size || userBookings.length) > 1;

...
const DASHBOARD_URL = process.env.DASHBOARD_URL || 'https://example.com/emi-dashboard/';
const DATA_CSV_URL = process.env.EMI_DATA_CSV_URL || '';

const DEFAULT_STATE_FILE = './.emi-dashboard-email-state.json';
...
const fetchLatestCsv = async () => {
  ...
  if (!DATA_CSV_URL) {
    throw new Error('Provide --csv-file or set EMI_DATA_CSV_URL.');
  }
  ...
  try {
    try {
      const response = await fetch(DATA_CSV_URL, { signal: controller.signal });
      if (!response.ok) {
        throw new Error(`Remote data source returned ${response.status} ${response.statusText}`);
      }
      const csv = await response.json();
      if (typeof csv !== 'string' || csv.trim().length === 0) {
        throw new Error('Remote data source latest_csv is empty or not a CSV string');
      }
      return csv;
    } catch (fetchError) {
      ...
      try {
        ...
        throw new Error(`Remote data source CSV fetch failed: ${fetchError.message}; curl fallback returned invalid JSON: ${parseError.message}`);
      }
    }
  } finally {
...
const getLatestDataDate = (csv) => {
  ...
  for (const row of parsed.data) {
    ...
    const rowDate = parseDate(row.timestamp || row.created_at || row.date);
    if (!rowDate) continue;
  ...
  try {
    ...
    const result = {
      ...
      email: {
        ...
        subject: 'EMI data updates',
        body: `Hello,

The EMI dashboard data has been updated through ${latest.latestDataLabel}.
...
--
You are receiving this automated email because you are subscribed to EMI Dashboard updates.`,
        bodyFile,
        attachmentFiles: [],
      },
      ...pending,
    ...
    if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
      ...
      await transporter.sendMail({
        from: `"EMI Dashboard" <${process.env.GMAIL_USER}>`,
        to: result.email.to,
    ...
    return {
      status: 'error',
      reason: `Unable to prepare EMI update email: ${error.message}`,
      fetchUrl: DATA_CSV_URL,
      stateFile,
    };
  }
};

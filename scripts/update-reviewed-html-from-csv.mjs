...
const update = async () => {
  ...
  html = replaceAllPairs(html, [
    ...
    [/\d+(?:\.\d+)?% of users anchor on a monthly EMI under Rs\. [0-9A-Z.]+, suggesting that a comfortable monthly payment is the primary driver for conversion\.(?:.*?total converted value\.)?/g, `${metrics.insights.emi} ${metrics.insights.tenure}`],
    ...
    [/Bookings of ₹[0-9.A-Z]+\+ make up .*?renewal nudges\./g, metrics.keyTakeaways.valueConcentration.text],
    ...
    [/\d+\/\d+ at ROI \d+(?:\.\d+)?%\+/g, `${formatNumber(metrics.keyTakeaways.roiTolerance.usersAt18)}/${formatNumber(metrics.keyTakeaways.roiTolerance.totalUsers)} at ROI 18%+`],
    [/Even though the system assigns ROI based on risk, \d+ users successfully converted at \d+(?:\.\d+)?% or higher\./g, `Even though the system assigns ROI based on risk, ${formatNumber(metrics.keyTakeaways.roiTolerance.usersAt18)} users successfully converted at 18% or higher.`],
